...
`Save to Google Drive` starts Google authorization before asking for a filename. When no valid Drive token is already in memory, Stickies redirects the current tab to Google, returns to `/Stickies/`, then resumes the same save action. First-time Drive saves check for an existing top-level `Stickies` folder that the app can use before asking to create one. If Stickies finds one or more matching folders, it tells the user, remembers the selected existing folder locally, and saves there without creating another folder. If no matching folder is found, Stickies asks the user to confirm creating a top-level `Stickies` folder in My Drive. After that, Stickies creates the `.stickies` file there, binds the browser project to that Drive file, and shows one copyable Drive file link plus an `Open Folder in Drive` action. Later saves update the bound Drive file and open the same Stickies folder from the success dialog.
...
`Open recent` is the preferred private cloud-open workflow because it reopens locally remembered Stickies Drive files without relying on broad Drive browsing. `Open from Public Gallery` is separate: it uses only `VITE_GOOGLE_API_KEY` to list public published files from the configured Stickies Public Gallery Drive folder and opens the selected item in the read-only public viewer. Google Picker folder selection, broad private Drive browsing, pasted folder links, and alternate save folders are not part of the save path. Stickies saves only to a top-level `Stickies` folder in My Drive, reusing an existing matching folder when the Drive API can see one and creating a new one only after an explicit confirmation when none is found.
...
- `Open from Public Gallery`: lists published Stickies files from the configured public gallery source and opens the selected file at `/Stickies/public/drive/<file-id>/` without OAuth or Drive binding.
- `Save to Google Drive`: updates the currently bound Drive file. If the project is not bound to Drive yet, it asks for a file name, reuses an existing top-level `Stickies` folder when found, confirms creation only when no matching folder is found, creates a `.stickies` file there, and binds the current project to that Drive file.
...
The editor's `Open from Public Gallery` action loads source definitions from [public/published/gallery-sources.json](/Users/jeremyguillory/Documents/vibecode-projects/SS%20React%20Flow%20Charts/public/published/gallery-sources.json). The current persistent source is `Stickies Public Gallery`, backed by Drive folder `1nvVvcCc4ML1Iyd4mXUHo4ZuKoRpPd4UT`. The gallery lists public `.stickies` and legacy JSON-compatible files newest-first through the Drive API using the browser API key, then navigates to the same `/Stickies/public/drive/<file-id>/` read-only viewer route. It does not request OAuth, create Drive folders, attach the editor to a Drive file, or update existing published snapshots.
