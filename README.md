...
`Open recent` is the preferred cloud-open workflow because it reopens locally remembered Stickies Drive files without relying on broad Drive browsing. Google Picker folder selection, broad Drive browsing, pasted folder links, and alternate save folders are not part of the save path. Stickies saves only to a top-level `Stickies` folder in My Drive, reusing an existing matching folder when the Drive API can see one and creating a new one only after an explicit confirmation when none is found.
